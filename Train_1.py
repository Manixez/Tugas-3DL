...
def create_vit_model(num_classes, freeze_backbone=True):
    ...
    if freeze_backbone:
        ...
        print("- Trainable: Only classification head (10 classes)")
        
        # Reset head untuk training dari awal (tanpa Softmax)
        model.model.head = nn.Linear(384, 10)  # 384 adalah dimensi feature, 10 adalah jumlah kelas CIFAR-10
...
def plot_metrics(train_losses, val_losses, train_accs, val_accs, save_dir='.'):
    ...
    # Menampilkan plot
    save_path = os.path.join(save_dir, 'hasil_training_ViT.png')
    plt.savefig(save_path)
    plt.close() 

    print(f"\nGrafik training telah disimpan sebagai '{save_path}'")

def plot_confusion_matrix(y_true, y_pred, class_names, filename='confusion_matrix_vit.png'):
    ...
    plt.title('Confusion Matrix - ViT Model (CIFAR-10)', fontsize=16, pad=20)
...
def main():
    ...
    img_size = 224  # Resize CIFAR-10 (32x32) to 224x224 for ViT
    ...
    best_model_path = os.path.join(script_dir, "best_vit_model.pth")
    ...
    confusion_matrix_path = os.path.join(script_dir, 'confusion_matrix_vit.png')
    ...
    # Print model summary
    print(f"\nModel Summary:")
    print(f"- Architecture: ViT-Small with ImageNet pretraining")
