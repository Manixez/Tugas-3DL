...
def create_cvt_model(num_classes, freeze_backbone=True):
    """Create CvT model with transfer learning"""
    model = CvT(num_classes=num_classes, pretrained=True)
    
    print(f"Loaded CvT-13 pretrained model from Hugging Face")
    
    if freeze_backbone:
        # Freeze semua layer kecuali classifier head
        for name, param in model.model.named_parameters():
            if 'classifier' not in name:  # Freeze semua kecuali classifier
                param.requires_grad = False
        
        print("Fine-tuning strategy:")
        print("- Frozen: All backbone layers (convolutional token embeddings, transformer stages)")
        print(f"- Trainable: Only classification head ({num_classes} classes)")
        
        # Print classifier structure
        print(f"Classifier structure: {model.model.classifier}")
...
def plot_metrics(train_losses, val_losses, train_accs, val_accs, save_dir='.'):
    ...
    # Menampilkan plot
    save_path = os.path.join(save_dir, 'hasil_training_CvT.png')
    plt.savefig(save_path)
    plt.close() 

    print(f"\\nGrafik training telah disimpan sebagai '{save_path}'")

def plot_confusion_matrix(y_true, y_pred, class_names, filename='confusion_matrix_cvt.png'):
    ...
    plt.title('Confusion Matrix - CvT-13 (CIFAR-10)', fontsize=16, pad=20)
...
def main():
    ...
    img_size = 224  # Resize CIFAR-10 (32x32) to 224x224 for CvT
    ...
    # Initialize CvT model with transfer learning
    print("\nInitializing CvT-13 model...")
    model = create_cvt_model(num_classes, freeze_backbone=True)
    print(model)
    ...
    # Base learning rate untuk CvT fine-tuning
    ...
    print(f"- Model: CvT-13 (pretrained)")
    ...
    best_model_path = os.path.join(script_dir, "best_cvt_model.pth")
    ...
    confusion_matrix_path = os.path.join(script_dir, 'confusion_matrix_cvt.png')
    ...
    # Print model summary
    print(f"\nModel Summary:")
    print(f"- Architecture: CvT-13 (Convolutional vision Transformer) with ImageNet pretraining")
